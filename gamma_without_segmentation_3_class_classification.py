...
if __name__ == '__main__':  #straight away go to this
    ...
    #inception_pred.compile(optimizer = tf.keras.optimizers.RMSprop(learning_rate=0.0002), loss=tf.keras.losses.CategoricalCrossentropy(from_logits = False) , metrics=['accuracy'])
        ...
        my_plots(history,"DenseNet201")
        filename = 'DenseNet201_model.sav'
        np.save('DenseNet201_model.npy',history.history)
        ...
        my_plots(history,"SE_INCEPTION_V3") #PLOTTING THE ACCURACY AND LOSS CURVE

        filename = 'SE_INCEPTION_V3_MODEL.sav'
        np.save('SE_INCEPTION_V3_MODEL.npy',history.history)
        ...
        my_plots(history,"se_squeeznet") #plotting the accuracy and loss curve

        filename = 'se_squeeznet_MODEL.sav' #file name with which model is saved
        np.save('se_squeeznet_MODEL.npy',history.history)
